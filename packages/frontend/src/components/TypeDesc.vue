...
<template>
  <span class="type" :data-sub="type.sub">
    ...
    <template v-if="type.sub === 'var'">
      <span class="node-var">{{ type.rigid ? type.customId : type.id }}</span>
    </template>
    <template v-else-if="type.sub === 'con'">
      <span class="node-con">{{ type.id }}</span>
    </template>
    <template v-else-if="type.sub === 'tuple'">
      (<template v-for="arg, i of type.args" :key="i">
        <TypeDesc :type="arg" :parent="type" />
        <span v-if="i + 1 < type.args.length" class="node-spaced-right">,</span>
      </template>)
    </template>
    ...
    <template v-else-if="type.sub === 'func'">
      <template v-for="arg, i of type.args" :key="i">
        <TypeDesc :type="arg" :parent="type" />
        <span v-if="i + 1 < type.args.length" class="node-op node-spaced">-&gt;</span>
      </template>
    </template>
